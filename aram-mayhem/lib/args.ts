 ...
 *
 * 「未知参数一律报错」的安全前提是「handler 不会读没声明的参数」——
 * 由 tools/audit-wiring.mjs 的 **F 节**保证（handler 读的每个 args.X 都声明在 schema 里）。
 * 少那一节，只要有一处 handler 读了个没声明的名字，所有客户端传那个参数都会被拦下来，
 * 等于把一个本来能用的功能打死。
 *
 * （这段注释原先引用的是「audit-wiring 的另一节」—— 写的时候那一节并不存在，
 * 是后来补的。引用了不存在的检查比不引用更坏：它会让下一个读代码的人以为已经有保护了。）
 */
