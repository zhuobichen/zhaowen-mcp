...
const DIFF_REGISTRY = [
  ...
  },
  {
    ...
    // 定位串原先写的是 `/4 个标准误/`（那句注释里），后来注释被改写、串就失效了 ——
    // 改成对准**代码本身**的那个表达式，注释怎么改都不影响。
    find: /const noise = 2 \* \(se\(/,
    value: 2, // 代码里是 `2 × (SE_下 + SE_上)`，等价于约 4 个「单个标准误」
    unit: "se",
    why: "全库唯一一个**跟着样本量走**的差值判据，而且用的是 `2×(SE_下+SE_上)` 而不是 `√(SE²+SE²)` —— 更保守。实测：六类标签下噪声能造出的最大偏离约 √(2·ln 6) ≈ 1.9 个标准误，而这里要 4 个，**是全库唯一一处门槛高于噪声线的判据**（其余几处都是这次才发现门槛偏低）",
    hedged: true,
  },
  {
...
const INLINE_REGISTRY = {
  ...
  "lib/report.ts:findings:inline:>=8": {
    tool: "（海斗 HTML 报告）",
    kind: "统计门槛",
    why: "撑着报告里「发现」那一段的几条结论。8 把的标准误约 18 个百分点，而「真命符文」是**从这些里挑胜率最高**——实测候选 10 个、噪声尺度约 36 个百分点，榜首 8 把 88%（3.2 倍）刚好压线。已按同一套补上候选数 / 噪声尺度 / 逐条倍数",
  },
  "lib/report.ts:findings:inline:>=5": {
    tool: "（海斗 HTML 报告）",
    kind: "统计门槛",
    why: "**同一个函数里 8 和 5 并存**，未说明为何不同。用在「英雄两极」（≥5 把里挑胜率最高最低）——实测候选 22 个、**噪声尺度约 39 个百分点**，而列出来的「顺手 80% / 该躲 29%」全在这个范围内。已在输出里写明「先当线索，别据此换英雄池」",
  },
