...
// 「赢得更多 / 赢面更大 / 反而更低」是**主张词**，只是不含「明显」二字 ——
// combat-profile 原先就是靠这个漏掉的：它写「这项做得多，赢得更多」，判据认不出来。
const STRONG = /可以当真|够下结论|明显|确凿|确定是|样本量够|很稳|相当明显|赢得更多|赢面更大|反而更低|反而低/;
// 有保留的措辞：出现这些说明工具自己已经打了折扣。
// 「先当倾向 / 别当结论 / 分不开 / 看不出」是这次一起补的：
// 放宽了 STRONG 就必须同时认这些**已经在打折扣**的说法，否则会把老实人一起罚。
const HEDGED =
  /样本少|样本不足|先当参考|仅供参考|不足以|可能|大概|谨慎|还不多|样本还|先当倾向|别当结论|分不开|看不出|卡在/;
...
export function weakEffect(verdict) {
  ...
  const pairs = [
    // 形态一：局数在前 —— 「36 局 47.2%」（tilt / contribution）
    ...[...verdict.matchAll(/(\d+)\s*(?:把|局)(?!及以上|以上)[^。；]{0,24}?(\d{1,3}\.\d)\s*%/g)].map((m) => ({
      n: Number(m[1]),
      p: Number(m[2]),
    })),
    // 形态二：百分比在前、局数在括号里 —— 「57.4%（40 局）」（combat-profile 补上局数后的写法）。
    // 少了这一条，combat 的结论一条都判不了（第一版就是这样，实测「11 处」里它只占了位置没被真的检查）。
    ...[...verdict.matchAll(/(\d{1,3}\.\d)\s*%[^。；]{0,6}?[（(]\s*(\d+)\s*(?:把|局)\s*[）)]/g)].map((m) => ({
      n: Number(m[2]),
      p: Number(m[1]),
    })),
  ];
...
if (process.argv.includes("--selftest")) {
  ...
  const weakCases = [
    ...
    // **主张词**不含「明显」二字，原先靠这个漏掉 —— combat-profile 就是。
    // 反证：把「赢得更多」从 STRONG 里拿掉，这条不会被抓。
    // 样本要给到**真撑不住**的量级：40 局 60% vs 40 局 45%，差 15 个百分点看着大，
    // 可标准误有 11 个点，z≈1.36 —— 那种情况下说「赢得更多」就是过度声称。
    // （第一版我给了 196/150 局，z=2.36 本来就该放过，用例反而测错了方向。）
    [
      "结论：队内第一 60.0%（40 局） / 第 4 名及以后合并 45.0%（40 局），差 15.0 个百分点 —— 这项做得多，赢得更多。",
      true,
    ],
    // 同一个工具老实说「看不出」时不能罚它 —— 放宽 STRONG 就必须同时认这些保留措辞
    [
      "结论：队内第一 51.1%（47 局） / 第 4 名及以后合并 58.5%（135 局），差 -7.5 个百分点（约是噪声的 0.88 倍） —— 看不出这项与胜负有关。",
      false,
    ],
    // 只报百分比、不报局数 → 判不出噪声，就不判（宁可不报也别猜）。
    // 这是 combat-profile 补上局数**之前**的形态。
    ["结论：队内第一 57.4% / 第 4 名及以后合并 44.7%，差 12.7 个百分点 —— 这项做得多，赢得更多。", false],
  ];
  let bad = 0;
  console.log("— 第一层：样本量 —");
