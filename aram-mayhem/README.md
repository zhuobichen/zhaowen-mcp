...
数据在构建阶段抓取并合并成本地快照，**运行时只读本地文件、不联网**。两处例外：`refresh_data` 会联网刷新数据；账号类工具（战绩/队友/对位/趋势/报告等）读本地 127.0.0.1 的客户端接口与腾讯 SGP，**客户端没开时会退回本地归档**，并在输出里标注数据来源与截止时间。全库只有 `send_champ_select_message` 一个写操作，且需要显式确认。

## 工具

| 工具 | 作用 |
|------|------|
| `get_help` | **不知道用哪个工具时先看这个**：按用户意图分场景给引导（选人/复盘/看状态/找毛病/查符文/对比/导出/维护），每场景列几句「你可能会说的话」 |
| `get_data_info` | 当前补丁、数据更新时间、各类条数、全部数据来源与已知数据出入 |
| `search_augments` | 搜/列符文：中文名、英文名、说明关键词；可按品质、在池状态过滤；默认按强度榜名次排 |
| `get_augment` | 单个符文详情：游戏内原文说明、品质、在池状态、两套胜率口径、强势英雄、所属羁绊、英雄评价与搭配，外加**本机实证**（在本地归档里被拿过多少次、胜率多少、和谁一起拿更赚/更亏） |
| `list_synergy_sets` | 列出羁绊（套装）：需要哪些符文、凑齐效果 |
| `analyze_synergy` | 羁绊推算：输入已有符文 → 能凑哪些羁绊、还差几件、缺的符文哪些还在池 |
| `list_champions` | 按胜率/梯队列英雄（含国服口径） |
| `get_champion_guide` | 英雄视角：梯队胜率 + 符文评价（带神级/强力/陷阱标签）+ 多件搭配 + 该英雄的强势符文 + 相关羁绊 + **本机实证**（这英雄拿到哪些符文胜率高，用「专属」列扣掉符文本身强度） |
...
| `get_my_teammates` | 队友/对手：谁和你一起打得最多、共同胜率、和谁打最稳；**和常一起打的人同队时你用哪些英雄打得最好**；对手维度的覆盖也一并说明 |
| `get_my_builds` | 出装：最常出的装备与胜率、出得多但胜率偏低该换的（按背包槽位统计，不是购买顺序） |
| `get_my_matchups` | 英雄对位：对面出现谁时你最容易输/最稳；用「残差」（减去该英雄自身胜率）区分「它本身强」和「你真的打不过」 |
| `compare_accounts` | 跨账号并排对比：局数/胜率/近期状态/KDA/伤害/常玩英雄/常用符文 + **符文偏好差异**（谁更爱拿什么）+ **两人都拿过的符文里胜率差最大的几件** + 一句结论 |
...
| `export_games_csv` | 导出对局明细 CSV（海斗/全部模式/云顶，39 列，含队内伤害名次与对面阵容构成），带 BOM，Excel 直接打开不乱码 |
...
| `get_my_tilt` | **连败/连胜之后的表现**：输了继续打是更差还是照样、赢了会不会飘；并说明胜率本来就会向 50% 回归 |
| `get_combat_profile` | **伤害以外的贡献**：控制时间/治疗量/最长存活/对目标伤害做成队内第几时胜率如何。负相关项会附两个对照（时长是否混淆、与「输出」的互斥度），不直接下因果结论 |
| `get_my_contribution` | **贡献度与胜负**：你在队里打第几（伤害/金币/KDA/补刀的**真实队内名次**）跟赢不赢有没有关系，并给「全队都顺」的对照来区分「你 carry」和「赢得顺所以数据好」 |
| `get_counter_items` | **对面阵容 → 出装**：面对「对面 ≥2 个坦克/战士/刺客…」时，归档里赢的一方最终带哪些成装、胜率如何（只统计成装，散件会得出反向因果的假结论） |
| `get_enemy_comps` | **对面阵容构成**：对面带坦克/战士/刺客/法师/射手/辅助标签的人有几个时你的胜率如何，并给出「有没有影响」的结论（标签出自 Riot 官方英雄数据） |
| `get_my_patches` | **按补丁看自己**：每个版本打了多少局、胜率/平均名次、常玩英雄；跨版本结论要求补丁样本 ≥15 局，够不上就直说比不出来 |
...
lib/queue-stats.ts    按队列拆分统计（海斗的多个队列、云顶排位与活动队列）
lib/patches.ts        按补丁看自己的表现（版本号来自 gameVersion，只有 SGP 会给）
lib/comps.ts          对面阵容构成（按 Riot 官方英雄定位标签粗分类）
lib/counters.ts       对面阵容 → 赢家出装（只用成装，排除散件干扰）
lib/contribution.ts   贡献度与胜负（队内名次 vs 胜率，含反向因果对照）
lib/tilt.ts           连败/连胜之后的表现（含均值回归的口径说明）
lib/checkup.ts        一键体检：汇总各维度结论并按偏离幅度排序
lib/game-detail.ts    单局详情（复盘用，支持按日期/英雄/gameId 选局）
lib/combat-profile.ts 伤害以外的贡献（字段审计查出来的那批「采集了没分析」的字段）
lib/report-md.ts      Markdown 版小结（第三种输出形态）
lib/report-compare.ts 双账号对比报告（逐周双线 / 英雄池并列 / 符文偏好差异）
...
四种报告/输出各管一头：**单人 HTML 报告**求全（海斗 14 图 + 深入分析节 / 云顶 10 图）· **双账号对比报告**求横向比（逐周双线、英雄池并列、符文偏好差异）· **Markdown 小结**求快看（一屏）· **CSV** 求可算（39 列）。

**海斗报告（14 张图 + 一节「深入分析」表）**：滚动 20 把胜率折线（红色带标出 4 连败以上的低谷）· **按星期几的胜率** · **队内伤害名次 → 胜率**（你在队里打第几跟赢不赢的关系）· **逐周场次与周胜率**（上柱=周局数、下折线=周胜率，空心点=该周不足 5 局）· **按补丁的胜率**（两行标签：客户端版本 / 赛季号；半透明柱=该补丁不足 15 局）· 按月胜率柱状 · 时段胜率柱状（几点打最稳）· 英雄胜率条形（50% 基准、背离着色）· **符文四象限散点**（版本胜率 × 你的胜率，右下方=版本强但你打不出，该换）· 符文「你 vs 版本」子弹图（表里带「其他人」列）· 常一起打的人 · 英雄×符文搭配胜率 · 出装与胜率 · **对位残差背离图**（中轴 0，左红=额外打不过）· 锐评与建议 · 最近 20 把明细。

**云顶报告（10 张图）**：名次分布（前四蓝/后四红）· 逐局名次折线（含 4.5 名基准）· **按星期几的平均名次** · **逐周场次与周平均名次** · **按补丁的平均名次**（与海斗同一口径，两个版本号都标）· **等级 × 名次散点**（高等级却差名次=运营问题）· 羁绊偏好 · 棋子偏好 · **装备偏好**（带它收场时的平均名次）· 羁绊组合与成绩 · 最近 20 局明细。同样支持 `--friend` 出别人的。
...
两个审计脚本都是**可重跑**的：
- `npm run audit:fields` —— 当前 **0 个未读字段**（34 个采集字段全部至少有一个模块在读）
...
- `npm run audit:recompute` —— **唯一一个有外部真值**的检查：不复用 lib/ 任何代码，
  ...
  当前**二十九个维度**全部吻合（34 项检查）：
  · 局数 306 把（4 个工具）· 胜率 53.3%（2 个）
  · 装备「无尽之刃 109 把」· 出装胜率「58%（63/109）」
  · 队友「自己的丁ding 208 局」· 对手「点名克星 奥瑞利安·索尔 12 把，你赢 4 把」
  · 符文「星界躯体：你 27 把（33.3%）vs 同批人 1437 行（50.9%）→ 差 -17.5」
  · 趋势分周「最近 58 把 50.0% / 之前 42 把 40.5%」（独立复现了「有效周 ≥5 局」的定义）
  · 补丁「16.3 共 81 局」（独立复现了 parsePatch 的三种形态 —— 那两个坑我在产品代码里踩过）
  · 战斗细节「承伤队内第一 57.4%（62/108）」· 贡献度「伤害队内第一 110 局 61.8%」
  · 符文协同「更万用的瞄准镜 + 战争交响乐 61 局 67.2%，协同 +17.4」
  · 对面阵容「对面有坦克 235 局 52.3%」· 队列拆分「[2400] 302 局 53.6%」
  · 排行榜「见过 28506 个账号，第一 火丶WSCan 15 局 73.3%」
  · 跨账号「我 306 局 53.3% / 丁ding 994 局 48.9%」
  · **云顶**「平均名次 4.22，吃鸡 68 次」（684 局）—— 云顶侧原先完全没对过账
  · 云顶棋子「尼菈 13 局 平均名次 2.46」· 云顶分补丁「15.24 共 252 局」
  · **CSV 导出**「306 行 39 列」+「结果」列 163 胜 143 负 —— 直接读导出文件数出来的
  · **报告产物**：Markdown 小结里写的「共 306 局，163 胜 143 负（53.3%）」
  · 云顶周趋势「前段 3.75 → 后段 4.56」· 云顶装备「烁刃 18 局 平均名次 3.17」
  · 符文实证段「坦克引擎 2658 次 51.1%」· 英雄实证段「亚索 388 局 57.0%」
  · HTML 报告页头「306 局 · 胜率 53.3%」（`npm run audit:recompute:html`，默认跳过 —— 要 90 秒）
  · 我的符文统计「306 把 / 163 胜」+「吸血习性 40 把 / 赢 24」
  · 云顶三星「凯南 ×22」（共 65 个棋子有三星）
  · 最近对局「第一条 = 霞 · 胜」，与归档里最新的那把一致

  审计**自己声明覆盖边界**：末尾会列出「故意不在对账范围内」的工具及原因 ——
  依赖实时状态的（段位/好友/当前登录态/选人会话）、写操作、联网刷新、纯静态文案，
  以及读打包图鉴快照而非我的数据的那些。不明说的话，「某项没出现在输出里」
  很容易被误读成「已经覆盖了」。

  这几处尤其值得查，因为各有容易算错的过滤：
  「其他人」基线要**剔除自己**（漏了会往「显得你有问题」的方向错）；
  队友要合并多局（「同一局数了两次」是最经典的错）；
  装备有价格门槛（鞋 900 / 其余 2000）；趋势有「≥5 局才算有效周」这个门槛；
  补丁有版本号解析（三种形态）；队内名次要求「至少 2 个队友」才排名；
  符文协同要同时算三组数（组合 + 两件单拿），且样本是**全归档所有参与者**不是只我自己；
  对面阵容要按角色标签**去重后数人头**（一名英雄可挂多个标签）；
  队列拆分统计的是**所有模式**，不能只筛海斗；
  排行榜要遍历**所有参与者**（不只我自己）按人聚合；
  云顶走的是**另一个归档文件**（tft-matches.json）和另一套字段（placement 而不是 win）；
  CSV 与 Markdown 两条对的是**文件产物**，不是工具的内存输出（读文件数行数、数列数、抽列值）。
  HTML 报告那条要跑 90 秒，所以放在 `--with-html` 后面、不拖慢日常套件。
  云顶周趋势那条暴露的不是算术错误而是**口径未说明**：工具用的是「各周平均名次的平均」
  （每周等权），不是按局数加权 —— 已在产品输出里补上这句口径说明。
- `npm run audit:confidence` —— 8 处结论的置信措辞都与样本相符（修掉 1 处过度声称，见下）。
  带 `--selftest`（6 条）。
- `npm run audit:direction` —— 检查了 9 处结论，**方向词与数字符号全部一致**；
  `get_my_matchups` 点名的克星/提款机确实是列表里的两个极值（-20.3 / 33.4）。
  这个审计带 `--selftest`：先确认它**真的会为矛盾的说法报警**（3 条合成的矛盾句被抓住、
  4 条正常的被放过）再去信它的「全部通过」—— 一个从没报过错的检查等于没有检查。
- `npm run audit:verdict` —— 6 个「结论 + 明细」结构的工具，**结论引用的每个数字都能在自己的明细里找到依据**
  （这条查出来 2 个真问题，见下）。
- `npm run audit:consistency` —— 「我的海斗总局数」在 9 个工具里都是 **306 把**、
  「整体胜率」在 7 个工具里都是 **53.3%** —— 一致，没分叉。
  这个检查当前的价值是**回归**：以后谁加了过滤口径不一致的模块，它会立刻报出来。
- `npm run audit:coldstart` —— 冷启动时 **25 个依赖个人数据的工具都会给可照做的提示**；
  健康模式下**没有工具被误补**这条提示（反向验证同样重要，只测「该出现时出现」不够）。
- `npm run audit:length` —— 当前**没有工具超过 120 行**（实测最短 1 行、中位 28 行、最长 75 行）。
  这个数字值得记下来：做这个检查之前我以为「有些工具返回太长」，量完发现**不成立** ——
  最长的 get_counter_items 也就 75 行（约 2300 字符），离需要精简模式还很远。（判据再严一层：分析结果只在 MCP 工具输出里 = 不问就看不到、没法对比留档）
- `npm run audit:wiring` —— 当前**无游离文件、无死代码**；41 个工具声明/分发/文档三方一致；39 个 npm script 指向的文件都存在
...
- **置信措辞不能超出样本撑得起的程度**：`get_my_matchups` 原先写「样本量够，这几条可以当真」，
  ...
  现在会明说「这两个英雄各自只遇到 12 把，样本太少：残差在这个量级上跟 0 区分不开，
  先当参考，别照着改出装」。是 `npm run audit:confidence` 查出来的。
- **结论的方向也要对**：`npm run audit:direction` 查两种「说反」——方向词与数字符号不一致
  （说「变强了」但差值是负的）、点名的对象不是极值。这比数字抄错更严重：
  数字错了读者能自己看出来，方向说反了读者会直接信。
  判据完全基于工具自己的输出，不依赖外部真值，所以改了算法这个检查依然有效。
- **结论必须能被自己的明细核对**：`npm run audit:verdict` 把结论行里引用的数字抽出来，去明细里找。
  查出的两个真问题：`get_my_contribution` 的结论说「排到第三及以后 128 局 49.2%」，但明细里只有
  分开的第 3/4/5 行；`get_my_trend` 的结论说「之前 4 个有效周 40.5%」，逐周表里也没有这一行。
  两个数字**都是对的**（我手算验过），但读者没法从表里核对 —— 已改成把聚合行也打出来。
  `get_combat_profile` 的差值同样改成把两个被减数都写出来（「队内第一 57.4% / 合并 44.7%，差 12.7」）。
- **同一个数在不同工具里必须一致**：`npm run audit:consistency` 把「我的海斗总局数」和「整体胜率」
  从 9 个工具的输出里抽出来比对。这件事必须查 —— 每个分析模块都自己 filter 一遍 `isMayhemGame`、
  自己数 win，是最容易悄悄分叉的地方；而一旦用户看到「这个说 306 把、那个说 304 把」，整份数据就不可信了。
  当前全部一致，所以这个检查现在是回归用的。
- **第一次装上时看到的东西同样重要**：冷启动审计把归档指到空目录、把客户端置为离线（`MAYHEM_LCU_OFFLINE=1`），跑一遍所有工具。
  原先 25 个依赖个人数据的工具里有 15 个返回的是**空壳**（标题 + 空表格 + 口径说明，没有「你该做什么」），
  看起来像装坏了。现在会统一补一句「打开客户端重试 / 跑 `npm run archive:sync`」。
  这个检查的判据改过三版：前两版都在嗅探输出文本（「出现过 0 把吗」「头部计数是否全 0」），两次都误判 ——
  最后改成**直接查数据**（客户端离线 + 归档为空），一次判断、两个方向都对。
- **「分析过」不等于「看得到」**：报告是能反复看、能对比、能留档的东西，MCP 工具输出不是。`npm run audit:surfaced` 查出 5 个分析（对面阵容构成、对面赢家出装、伤害以外的贡献、连败表现、按队列拆分）只活在工具输出里 —— 已在海斗报告加一节「深入分析」，用表格列出（这几项结论常是「没影响」，画图反而误导）。
- **「被读过」不等于「被分析过」**：`totalDamageTaken`（承伤）和 `champLevel`（局内等级）早就采了、也进了导出的 CSV，但没有任何模块对它们做判断 —— 是更严的那轮审计（`audit:analysis`）查出来的，判据从「有模块引用」收紧到「有模块拿它做判断」。补上分析后，局内等级这项又被发现**不可用**：大乱斗共享经验，83% 的局你都排队内第一，名次没有区分度，所以工具会明确拒绝下结论。
- **归档里有采了但不分析的字段，原因写在输出里**：`visionScore` 归档 38610 行里只有 653 行非零（2%）—— 海斗没有眼位系统；`timeSpentDead` 0 行有值 —— 采集列表里声明了但数据源（SGP/LCU）实际没给。这两个不静默跳过，`get_combat_profile` 会说明为什么不分析。
- **统计出装要先把散件剔掉**：散件（短剑 250、红水晶 400）留在最终背包里，多半说明那局结束得早或没发育起来 —— 赢的队结束得快，反而更容易剩散件。不剔掉会算出「打坦克时出短剑胜率 62%」这种反向因果。判据用价格：成装最低 2000（瑞格之灯/烈焰之炬），散件最高 1300（暴风之剑）、鞋最高 1250，2000 是一条干净的界。
- **「对面阵容构成」用的是官方定位标签，粒度很粗**：一名英雄可挂多个标签（亚索是战士+刺客），所以 `get_enemy_comps` 数的是「几个人的标签里有它」，不是「几个纯职业」。而且这层分析问你「对面阵容强不强」，本来就会影响胜率 —— 两边同时在变，所以工具会先算一个噪声线，差距没超过噪声就直说「没影响」，不硬讲结论。
- **英雄对位的「残差」是近似**：社区站胜率统计的是该英雄在**己方**时的表现，这里用它去减「它在**对面**时你的胜率」，两者口径不完全对称，只用来把「它本身强」从对位数据里大致扣掉。
- **云顶的版本字段是构建串**：`game_version` 形如 `Linux Version 16.18.817.4437 … <Releases/16.18>`，而更早的对局写的是 `TFT Unreal Version ?.?.?.?` —— 那种**真的没有版本号**，解析成 null 并在图下写明有多少局没计入，不编一个。
- **补丁号有两个写法，别搞混**：对局记录里的 `gameVersion` 是客户端版本（如 `16.18`），玩家说的是赛季号（`26.18`）—— Riot 的赛季号比客户端大版本大 10。`get_my_patches` 两个号都标出来。另外 `gameVersion` 只有 SGP 会给，本地客户端的历史摘要没有这个字段；`npm run archive:sync` 会用 SGP 把已有对局的版本号补回来（实测 3895 局全部补齐）。
