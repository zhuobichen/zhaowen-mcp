...
三种输出形态各管一头：**HTML 报告**求全（14/10 张图）· **Markdown 小结**求快看（一屏）· **CSV** 求可算（39 列，自己丢进 Excel）。
...
- `npm run audit:surfaced` —— 当前 **12/12 个分析都进了报告**（判据再严一层：分析结果只在 MCP 工具输出里 = 不问就看不到、没法对比留档）
