...
**海斗报告（11 张图）**：滚动 20 把胜率折线（红色带标出 4 连败以上的低谷）· **逐周场次与周胜率**（上柱=周局数、下折线=周胜率，空心点=该周不足 5 局）· 按月胜率柱状 · 时段胜率柱状（几点打最稳）· 英雄胜率条形（50% 基准、背离着色）· **符文四象限散点**（版本胜率 × 你的胜率，右下方=版本强但你打不出，该换）· 符文「你 vs 版本」子弹图（表里带「其他人」列）· 常一起打的人 · 英雄×符文搭配胜率 · 出装与胜率 · **对位残差背离图**（中轴 0，左红=额外打不过）· 锐评与建议 · 最近 20 把明细。
