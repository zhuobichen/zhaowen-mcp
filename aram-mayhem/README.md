...
**海斗报告**：滚动 20 把胜率折线（红色带标出 4 连败以上的低谷）· 按月胜率柱状 · 时段胜率柱状（几点打最稳）· 英雄胜率条形（50% 基准、背离着色）· 符文「你 vs 版本」子弹图 · 英雄×符文搭配胜率 · 锐评与建议 · 最近 20 把明细。

**云顶报告**：名次分布（前四蓝/后四红）· 逐局名次折线（含 4.5 名基准）· 羁绊与棋子偏好（按平均名次排序）· 最近 20 局明细。
